...
class HouseDeviceGarageActivity : AppCompatActivity() {
    ...
    public fun onButtonUpGarageClic(view: View) {
        ...
        val command = "OPEN"
        sendCommand(garage.id, command, ::onGarageCommandResult)
    }

    public fun onButtonDownGarageClic(view: View) {
        val garage = devices.find { it.type == "garage door" }
        if (garage == null) {
            Toast.makeText(this, "Porte garage non trouvée", Toast.LENGTH_SHORT).show()
            return
    ...
    public fun onButtonStopGarageClic(view: View) {
        val garage = devices.find { it.type == "garage door" }
        if (garage == null) {
            Toast.makeText(this, "Porte garage non trouvée", Toast.LENGTH_SHORT).show()
            return
